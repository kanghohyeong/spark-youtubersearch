...
def sources(urls):
    ...
    for i in range(1):
        driver = wd.Chrome(executable_path = "chromedriver.exe")
        driver.get('https://www.youtube.com'+urls[i])
        last_page_height = driver.execute_script("return document.documentElement.scrollHeight")
        while True:
            driver.execute_script("window.scrollTo(0, document.documentElement.scrollHeight);")
            time.sleep(1.0)
            ...
            if (new_page_height == last_page_height) | (count == 10):
                break
            count += 1
            last_page_height = new_page_height
...
name = 'ASMR_PPOMO'
title , urls = get_urls('UCAtFkapSeoEGPxm5bC3tvaw')
htmls = sources(urls)
result = comment(name,htmls)
